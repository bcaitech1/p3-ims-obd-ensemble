...
@PIPELINES.register_module()
class CopyPaste(albumentations.DualTransform):
    def __init__(
        self,
        blend=True,
        sigma=3,
...
def copy_paste_class(dataset_class):
    def _split_transforms(self):
        ...
        if split_index is not None:
            ...
            #recreate transforms
            self.transforms = albumentations.Compose(pre_copy, bbox_params, keypoint_params, additional_targets=None)
            self.post_transforms = albumentations.Compose(post_copy, bbox_params, keypoint_params, additional_targets=None)
            self.copy_paste = albumentations.Compose(
